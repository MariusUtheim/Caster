...
#error Unsupported platform
	...
	double caster_get_length(double instance, double method)
	{
		if (!initialized) return CASTER_NOT_INITIALIZED;
		...
		if (method == 1) 
			return s->get_length_seconds(inst == 0 ? 0 : inst - 1);
		if (method == 2)
			return s->get_length_samples(inst == 0 ? 0 : inst - 1);
		if (method == 3)
			return s->get_length_bytes(inst == 0 ? 0 : inst - 1);
		else return 0;
	}
	
	double caster_get_position(double instance, double method)
	{
		if (!initialized) return CASTER_NOT_INITIALIZED;
		...
		GET_SAMPLE(instance, &s, &inst);
		if (method == 1) 
			return s->get_position_seconds(inst == 0 ? 0 : inst - 1);
		if (method == 2)
			return s->get_position_samples(inst == 0 ? 0 : inst - 1);
		if (method == 3)
			return s->get_position_bytes(inst == 0 ? 0 : inst - 1);
		else return 0;
	}
	
	double caster_set_position(double instance, double value, double method)
	{
		if (!initialized) return CASTER_NOT_INITIALIZED;
		...
		if (instance == GM_ALL)
		{
			...
			{
				if (method == 1)
					s->set_position_seconds((float)value);
				if (method == 2)
					s->set_position_samples((float)value);
				if (method == 3)
					s->set_position_bytes((float)value);
			}
		...
		{
			if (method == 1)
				s->set_position_seconds(inst-1,(float)value);
			if (method == 2)
				s->set_position_samples(inst-1,(float)value);
			if (method == 3)
				s->set_position_bytes(inst-1,(float)value);
		}
		return 0;
	}
